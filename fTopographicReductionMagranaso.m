function Gzz = fTopographicReductionMagranaso(xObs,yObs,zObs,triangles,Points,density)
%fTopographicReduction
%            
    Gzz = zeros(length(zObs),1);
    
    % Calculate the topographic reduction
    % ---  created by Matthias Bär ---
    h=waitbar(0,'Calculating with Magranaso...');
    for i = 1:length(zObs)    
        X       = xObs(i);
        Y       = yObs(i);
        points = Points;
        points(:,3) = points(:,3)-zObs(i);
...
tan2ang=1/(An*Bn*Cn+dotab*Cn+dotac*Bn+dotbc*An)*deter;
ang=2*atan(tan2ang);
end